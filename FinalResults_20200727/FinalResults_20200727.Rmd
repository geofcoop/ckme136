...
```{r}
tor_games = read.csv("tor_games_20200727.csv")  # read csv file 
tor_games
```
...
## 2a. Train model - baseline - linear regression
...
## 2b. Train model - baseline - k-NN regression

# remove outcome variable from training set
library(dplyr)
actuals_knn <- tor_games_training %>% select(tor_runs)
tor_games_knn_training <- tor_games_training %>% select(-tor_runs)

# set the seed to make the partition reproducible
set.seed(1234)

# 75% of the sample size
smp_size <- floor(0.75 * nrow(tor_games_knn_training))
train_ind <- sample(seq_len(nrow(tor_games_knn_training)), size = smp_size)

# creating test and training sets that contain all of the predictors
knn_train <- tor_games_knn_training[train_ind, ]
knn_test <- tor_games_knn_training[-train_ind, ]
actuals_knn_train <- actuals_knn[train_ind, ]
actuals_knn_test <- actuals_knn[-train_ind, ]

### train regression model - WHIP, DER, OPS game values
library(FNN)
reg_results <- knn.reg(knn_train, knn_test, actuals_knn_train, k = 12)
print(reg_results)

plot(actuals_knn_test, reg_results$pred, xlab="Actual (y)", ylab="Predicted (ŷ)")
abline(lm(reg_results$pred~actuals_knn_test), col="red")

#mean square prediction error
mean((actuals_knn_test - reg_results$pred) ^ 2)

#mean absolute error
mean(abs(actuals_knn_test - reg_results$pred))

## 2c. Train model - baseline - regression trees

# grow tree
library(rpart)
fit <- rpart(tor_runs~game_park_factor+tor_home+opp_pitcher_whip+opp_team_der+drurb001_ops+bichb001_ops+biggc002_ops+davij007_ops+fishd001_ops+galvf001_ops+gricr001_ops+guerv002_ops+hernt002_ops+gurrl001_ops+jansd001_ops+maill001_ops+mcgur002_ops+mckib001_ops+pillk001_ops+smoaj001_ops+sogae001_ops+tellr001_ops, method="anova", data=tor_games_training)
...
# plot tree
plot(fit, uniform=TRUE, main="Regression Tree for Runs Scored")
text(fit, use.n=TRUE, all=TRUE, cex=.8)

# create attractive postcript plot of tree
psfile = paste(wd,"/tree2.ps",sep="")
post(fit, file = psfile, title = "Regression Tree for Runs Scored")
